...
class FilterModel extends Model
{
    ...
    public function filter($text, $replace = "***")
    {
        ...
        {
            $word = '/'.$word.'/ui';
            $text = preg_replace($word, $replace, $text);
        }

        return $text;
    }

    public function custom_filter($text, $words, $replace = "***")
    {
        $words = base64_decode($words);
        $words = explode(",", $words);
    ...
    public function url_to_html_element($text)
    {
        $pattern = '/url\[([\S]*)\]/ui';

        return preg_replace($pattern, '<a href="/go?url=\\1" target="_blank" title="\\1">(ссылка удалена)</a>', $text);
    }

    public function url_filter($text)
    {
        $pattern = '/(https?:\/\/)?([\S]+)\.([\S]{2,6})([\S]*)*\/?/ui';

        return preg_replace($pattern, 'url[\\0]', $text);
    }

    public function smileFilter($text)
    {
        $pattern = '/<img class="smile-text" platform=["]([a-z]+)["] smile-id=["]([0-9]+)["] src="([\S]+)">/ui';

        return preg_replace($pattern, 'smile[\\1_\\2]', html_entity_decode($text));
    }

    public function encodeSmiles($text)
    {
        model("Smile");

        $pattern = '/smile\[([a-z]+)_([0-9]+)\]/ui';
        preg_match_all($pattern, $text, $smiles);

        foreach($smiles[2] as $key => $smile)
        {
            $smile_data = $this->SmileModel->getSmile($smiles[1][$key], $smile);
            $img = (empty($smile_data['smile_image'])) ? "https://static-cdn.jtvnw.net/emoticons/v1/".$smile_data['smile_image_id']."/1.0" : $smile_data['smile_image'];
            $text = preg_replace('/smile\['.$smiles[1][$key].'_'.$smile.'\]/ui', '<img class="smile-text" src="'.$img.'">', $text);
        }
        return $text;
    }

    public function spacefilter($text)
    {
        $pattern = '/&nbsp/ui';

        return preg_replace($pattern, ' ', html_entity_decode($text));
    }
}