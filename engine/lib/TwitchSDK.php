...
class TwitchSDK
{
	...
	const URL_TWITCH = 'https://id.twitch.tv/';    
    ...
    public function __construct($config = array())
    {
        if (!in_array('curl', get_loaded_extensions())) {
            throw new Debug('cURL extension is not installed and is required');
        }
        ...
        if (!empty($config)) {
            if ($this->configValidate($config) === true) {
                ...
                throw new Debug('Wrong Twitch API config parameters');
            }
        }
    }
    ...
    public function authLoginURL($scope)
    {
        ...
        return self::URL_TWITCH . self::URI_AUTH . $query_string . "&scope=" . $scope;
    }
    ...
    private function generalRequest($params, $uri, $method = 'GET', $postfields = null)
    {
        ...
        if (curl_errno($crl) && $this->throw_curl_errors === true) {
            throw new Debug(curl_error($crl), curl_errno($crl));
        }
    ...
    {
        throw new Debug('Cannot call authenticate functions without valid API configuration');
    }
}
